...
export default async function HomePage({ searchParams }: { searchParams: Promise<{ categoria?: string }> }) {
  ...
  return (
    <>
      <style>{`
        .hero-section{position:relative;background:radial-gradient(120% 140% at 18% 0%, #FCEFE7 0%, #FBF6F0 38%, #FFFFFF 72%);overflow:hidden}
        .hero-glow{position:absolute;top:-20%;right:-8%;width:52%;padding-bottom:52%;border-radius:50%;background:radial-gradient(circle, rgba(214,40,40,0.13) 0%, rgba(214,40,40,0) 70%);pointer-events:none}
        .hero-wrap{position:relative;padding:44px 36px 32px;display:flex;justify-content:center}
        @media(max-width:768px){.hero-wrap{padding:32px 20px 22px}}
        ...
        .strip{position:relative;display:grid;grid-template-columns:1.1fr .8fr 1fr;gap:32px;padding:28px 36px 40px;align-items:start}
        @media(max-width:860px){.strip{grid-template-columns:1fr;gap:20px;padding:22px 20px 32px}}
